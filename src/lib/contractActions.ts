...
const CONTRACT_ABI = [
  ...
  "function manualDisbursement() external",
  ...
  "function canDisburseOnHighThreat() external view returns (bool)",
...
// Send flood metrics to contract (NO LONGER REQUIRES OWNERSHIP)
...
// Send direct deposit to contract (will be treated as sponsor deposit)
export async function sendDirectDeposit(amount: string = "0.1"): Promise<void> {
  try {
    const provider = new ethers.BrowserProvider(getEthereumProviderOrThrow());
    const signer = await provider.getSigner();
    ...
    console.error("Error sending direct deposit:", error);
    throw new Error(error.reason || error.message || "Transaction failed");
  }
}
...
export async function addBeneficiary(
  ...
  try {
    ...
    if (error.code === "CALL_EXCEPTION" && error.data?.includes("118cdaa7")) {
      throw new Error("Access denied: Only the contract owner can add beneficiaries");
    }
...
// Manual disbursement (owner only)
export async function manualDisbursement(): Promise<void> {
  try {
    const contract = await getContract();
    const tx = await contract.manualDisbursement();
    await tx.wait();
  } catch (error: any) {
    console.error("Error triggering manual disbursement:", error);
    
    if (error.code === "CALL_EXCEPTION" && error.data?.includes("118cdaa7")) {
      throw new Error("Access denied: Only the contract owner can trigger manual disbursement");
    }
    
    if (error.reason?.includes("No beneficiaries available")) {
      throw new Error("Cannot disburse funds: No beneficiaries have been added yet");
    }
...
// Read beneficiary info
export async function readBeneficiaryInfo(): Promise<{
  beneficiaryCount: number;
  canDisburse: boolean;
}> {
  try {
    console.log("Reading beneficiary info from:", CONTRACT_ADDRESS);
    const contract = await getReadOnlyContract();

    try {
      const [beneficiaryCount, canDisburse] = await Promise.all([
        contract.getBeneficiaryCount().catch(() => BigInt(0)),
        contract.canDisburseOnHighThreat().catch(() => false),
      ]);

      return {
        beneficiaryCount: Number(beneficiaryCount),
        canDisburse: Boolean(canDisburse),
      };
    } catch (err) {
      console.log("Failed to read beneficiary info, returning defaults:", err);
      return {
        beneficiaryCount: 0,
        canDisburse: false,
      };
    ...
    console.error("Error reading beneficiary info:", error);
    throw new Error(`Failed to read beneficiary info: ${error.reason || error.message}`);
  }
}
...
export async function debugContract(): Promise<{
  ...
  try {
    ...
    let owner, userAddress, isOwner, beneficiaryCount, canDisburse;
    ...
    try {
      ...
      if (code !== "0x") {
        ...
        [owner, beneficiaryCount, canDisburse] = await Promise.all([
          contract.owner().catch(() => null),
          contract.getBeneficiaryCount().catch(() => BigInt(0)),
          contract.canDisburseOnHighThreat().catch(() => false),
        ]);
        
        isOwner = owner && userAddress.toLowerCase() === owner.toLowerCase();
        beneficiaryCount = Number(beneficiaryCount);
      }
    } catch (err) {
      console.log("Could not get contract info:", err);
    }
