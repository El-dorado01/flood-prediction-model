...
contract FloodPredictor is Ownable {
    ...
    // Handle direct deposits to contract (automatically treated as sponsor deposits)
    ...
    // Fallback function for any call with data
    ...
    // Update metrics - REMOVED onlyOwner modifier (anyone can update)
    ...
    function updateThreatLevel() internal {
        ...
        if (waterLevel > WATER_LEVEL_THRESHOLD &&
            ...
            // Only disburse funds if there are beneficiaries
    ...
    function disburseFundsToBeneficiaries() internal {
        require(beneficiaries.length > 0, "No beneficiaries available for disbursement");
    ...
    // Manual disbursement function (only owner, requires beneficiaries)
    function manualDisbursement() external onlyOwner {
        require(beneficiaries.length > 0, "No beneficiaries available for disbursement");
        disburseFundsToBeneficiaries();
    }
    ...
    // View function to get beneficiary count
    function getBeneficiaryCount() external view returns (uint256) {
        return beneficiaries.length;
    }

    // View function to check if high threat disbursement is possible
    function canDisburseOnHighThreat() external view returns (bool) {
        return beneficiaries.length > 0 && (totalSponsorFunds + totalInvestorFunds) > 0;
    }
}